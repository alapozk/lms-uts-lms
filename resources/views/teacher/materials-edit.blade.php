...
<div class="wrap">
  <div class="box">
    <h2 style="font-weight:900;margin-bottom:12px">✏️ Edit Materi — {{ $course->title }}</h2>
    ...
    <form method="POST" action="{{ route('teacher.materials.update', [$course->id, $material->id]) }}" enctype="multipart/form-data">
      @csrf @method('PUT')

      <label class="label">Judul</label>
      <input class="input" name="title" value="{{ old('title', $material->title) }}" placeholder="Judul materi" required>

      <label class="label" style="margin-top:12px">Ganti File (opsional)</label>
      <input class="file" type="file" name="file" accept=".pdf,.ppt,.pptx,video/*">
      <div class="meta">
        Saat ini: {{ strtoupper($material->extension) }},
        {{ number_format($material->size/1024,0) }} KB —
        <a href="{{ \Storage::disk('public')->url($material->file_path) }}" target="_blank">Lihat / Unduh</a>
      </div>

      <div class="row">
        <button class="btn primary" type="submit">💾 Simpan Perubahan</button>
        <a class="btn" href="{{ route('teacher.materials.show', [$course->id, $material->id]) }}">← Batal</a>
      </div>
    </form>
  </div>
</div>
